...
wakeUp(alarm)
    ...
    .then(isIngredientsExist => {
        console.log(isIngredientsExist);
        return goToJob(true);
    })
    .then(traffic => {
        console.log(traffic);
    ...
    .then(workNum => {
        console.log(workNum);
        return learnProgramming('java');
    })
    .then(language => {
        console.log(language);
        return chillWithGirlfriend('Mortal Kombat');
    })
    .then(games => {
        console.log(games);
        return readBooks('так говорил заратустра');
    })
    .then(book => {
        console.log(book);
        return sleep(12);
